...
public class DataOperation {
    ...
    public Future<List<UserData>> newUserDataOpertion(String path, String name, String channel) {
        ...
        Future<List<UserData>> future = io.vertx.core.Future.future();
        vertx.executeBlocking(future1 -> {
            List user = excelRead.readExcel(path);
            future1.complete(user);
        }, asyncResult -> {
            if (asyncResult.succeeded()) {
                List user = (List) asyncResult.result();
                if (user == null || user.isEmpty()) {
                    ...
                    for (int i = 0; i < user.size(); i++) {
                        ...
                        try {
                            ...
                            userData.setDailyNewUser(Integer.parseInt(new BigDecimal(Double.valueOf(String.valueOf(arrayList.get(1)).trim())).setScale(0, BigDecimal.ROUND_HALF_UP).toString()));
                        } catch (Exception e) {
                            userData.setDailyNewUser(0);
                        }
                        try {
                            userData.setDailyActivityUser(Integer.parseInt(new BigDecimal(Double.valueOf(String.valueOf(arrayList.get(2)).trim())).setScale(0, BigDecimal.ROUND_HALF_UP).toString()));
                        } catch (Exception e) {
                            userData.setDailyActivityUser(0);
                        }
                        try {
                            userData.setStartupTime(Integer.parseInt(new BigDecimal(Double.valueOf(String.valueOf(arrayList.get(3)).trim())).setScale(0, BigDecimal.ROUND_HALF_UP).toString()));
                        } catch (Exception e) {
                            userData.setStartupTime(0);
                        }
                        userData.setSingleUseTime(String.valueOf(arrayList.get(4)));
                        String retention = String.valueOf(arrayList.get(6)).substring(0, String.valueOf(arrayList.get(6)).indexOf("%"));
                        Double doubleRentention;
                        if (Judgement.IsNumber(retention)) {
                            doubleRentention = Double.valueOf(retention) / 100;
                        } else {
    ...
    public List<AdData> changeyixin(String str, List<List<String>> matchinglist) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (eCpm == null || eCpm.length() == 0) {
                ...
                adData.setEcpm(Judgement.formatDouble3(Double.valueOf(eCpm)));
            }
    ...
    public Future<List<AdData>> fourthreeOperation(String path, List<List<String>> matchinglist) {
        ...
        vertx.executeBlocking(future1 -> {
            ...
            if (list == null || list.isEmpty()) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    List array = (ArrayList) list.get(i);
                    AdData adData = ToAdData.fourthreeToAdData(array, matchinglist);
                    adDataList.add(adData);
                }
            }
            future1.complete(adDataList);
    ...
    public Future<List<AdData>> toutiaoOperation(String path, List<List<String>> matchinglist) {
        ...
        vertx.executeBlocking(future1 -> {
            ...
            if (list == null || list.isEmpty()) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    List array = (ArrayList) list.get(i);
                    adDataList.add(i, ToAdData.toutiaoToAdData(array, matchinglist));
                }
            }
            future1.complete(adDataList);
        }, asyncResult -> {
    ...
    public Future<List<AdData>> newjiuyouOperation(String path, List<List<String>> matchinglist) {
        ...
        vertx.executeBlocking(future1 -> {
            ...
            if (list == null || list.isEmpty()) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    List array = (ArrayList) list.get(i);
                    AdData adData = ToAdData.newjiuyouToAdData(array, matchinglist);
                    adDataList.add(adData);
                }
            }
            future1.complete(adDataList);
    ...
    public List<TotalVO> listToTotalVO1(List<JsonObject> userList, List<JsonObject> adList, List<Long> longdates, String name) {
        List<TotalVO> list = new ArrayList<>();
        for (int j = 0; j < longdates.size(); j++) {
            TotalVO totalVO = new TotalVO();
            List<ChannelVO> channelVOS = new ArrayList<>();
            Long oneday = longdates.get(j);
            String channel = "oppo";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "oppo"));
            channel = "xiaomi";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "小米"));
            channel = "vivo";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "vivo"));
            channel = "meizu";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "魅族"));
            channel = "qihoo";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "qihoo"));
            channel = "samsung";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "三星"));
            channel = "lenovo";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "联想"));
            channel = "jiuyou";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "九游"));
            channel = "4399";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "4399"));
            channel = "yyh";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "应用汇"));
            channel = "yyb";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "应用宝"));
            channel = "baidu";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "百度"));
            channel = "coolpad";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "酷派"));
            channel = "jinli";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "金立游戏"));
            channel = "huawei";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "华为"));
            channel = "zhongxing";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "中兴"));
            channel = "taptap";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "taptap"));
            channel = "haoyou";
            channelVOS.add(ToChannelVO1(name, oneday, channel, userList, adList, "好游快爆"));
    ...
    public List<TotalVO> listToTotalVO2(List<JsonObject> userList, List<JsonObject> adList, List<Long> longdates, String name, List names) {
        ...
        for (int j = 0; j < longdates.size(); j++) {
            ...
            Long oneday = longdates.get(j);
            String channel = "oppo";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "oppo"));
            channel = "xiaomi";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "小米"));
            channel = "vivo";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "vivo"));
            channel = "meizu";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "魅族"));
            channel = "qihoo";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "qihoo"));
            channel = "samsung";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "三星"));
            channel = "lenovo";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "联想"));
            channel = "jiuyou";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "九游"));
            channel = "4399";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "4399"));
            channel = "yyh";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "应用汇"));
            channel = "yyb";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "应用宝"));
            channel = "baidu";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "百度"));
            channel = "coolpad";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "酷派"));
            channel = "jinli";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "金立游戏"));
            channel = "huawei";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "华为"));
            channel = "zhongxing";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "中兴"));
            channel = "taptap";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "taptap"));
            channel = "haoyou";
            channelVOS.add(ToChannelVO2(name, names, oneday, channel, userList, adList, "好游快爆"));
    ...
    public UserVO userJsonToUserVO1(List<JsonObject> userList, String channel, Long oneday,List names) {
        ...
        Integer dnu=0;
        Integer dau=0;
        Integer startup_time =0;
        String single_use_time =  "--%";
        Double retention = 0.0;
        for (int i = 0; i < userList.size(); i++) {
            JsonObject jsonObject = userList.get(i);
            if (channel.equals(jsonObject.getString("channel")) && oneday.equals(jsonObject.getLong("date"))&& InitConf.validNameList(names,jsonObject.getString("app_name"))) {
                dnu = jsonObject.getInteger("dnu")==null?0:jsonObject.getInteger("dnu")+dnu;
                dau = jsonObject.getInteger("dau")==null?0:jsonObject.getInteger("dau")+dau;
                startup_time = jsonObject.getInteger("startup_time")==null?0:jsonObject.getInteger("startup_time");
                single_use_time = jsonObject.getString("single_use_time")==null?"--%":jsonObject.getString("single_use_time");
                retention = jsonObject.getDouble("retention")==null?0.0:jsonObject.getDouble("retention");
            }
            userVO.setNewUser(dnu);
            userVO.setActivityUser(dau);
            userVO.setStartupTime(startup_time);
            userVO.setSingleUseTime(single_use_time);
            userVO.setRetention(Judgement.formatDouble4(retention));
        }
        return userVO;
    }
    ...
    public AdDataVO adJsonToAdDataVO(JsonObject jsonObject) {
        ...
        Double ecpm = 0.0;
        try {
            ecpm = Judgement.formatDouble3(earned / impression * 1000);
        } catch (Exception e) {
            ecpm = 0.0;
        }
     ...
     * @return
     */
    public List<AdDataVO> adDataVOToAdDataVOlist1(List names,List<JsonObject> adList, Long oneday, String channel, String platform) {
        ...
        for (int i = 0; i < adList.size(); i++) {
            AdDataVO adDataVO = null;
            JsonObject jsonObject = adList.get(i);
            Boolean banner_bool =InitConf.validNameList(names,jsonObject.getString("app_name")) &&oneday.equals(jsonObject.getLong("date")) && jsonObject.getString("channel").contains(channel) && platform.equals(jsonObject.getString("platform")) && jsonObject.getString("advertising_type").contains("横幅");
            if (banner_bool) {
                if (adDataVOList.get(0).getIncome() == 0.0) {
                    adDataVO = adJsonToAdDataVO(jsonObject);
                    adDataVOList.set(0, adDataVO);
            ...
            Boolean inline_bool =InitConf.validNameList(names,jsonObject.getString("app_name")) && oneday.equals(jsonObject.getLong("date")) && jsonObject.getString("channel").contains(channel) && platform.equals(jsonObject.getString("platform")) && jsonObject.getString("advertising_type").contains("插屏");
            if (inline_bool) {
                if (adDataVOList.get(1).getIncome() == 0.0) {
                    adDataVO = adJsonToAdDataVO(jsonObject);
                    adDataVOList.set(1, adDataVO);
            ...
            Boolean splish_bool =InitConf.validNameList(names,jsonObject.getString("app_name")) && oneday.equals(jsonObject.getLong("date")) && jsonObject.getString("channel").contains(channel) && platform.equals(jsonObject.getString("platform")) && jsonObject.getString("advertising_type").contains("开屏");
            ...
            Boolean video_bool =InitConf.validNameList(names,jsonObject.getString("app_name")) && oneday.equals(jsonObject.getLong("date")) && jsonObject.getString("channel").contains(channel) && platform.equals(jsonObject.getString("platform")) && jsonObject.getString("advertising_type").contains("视频");
     ...
     * @return ChannelVO
     */
    public ChannelVO ToChannelVO1(String name, Long oneday, String channel, List<JsonObject> userList, List<JsonObject> adList, String plastform) {
        ChannelVO channelVO = new ChannelVO();
        String onedaysdf = Transform.transForDate(oneday);
        ...
        List<AdDataVO> qudao = adDataVOToAdDataVOlist(adList, oneday, channel, plastform);
        List<AdDataVO> yixin = adDataVOToAdDataVOlist(adList, oneday, channel, "移信");
        List<AdDataVO> toutiao = adDataVOToAdDataVOlist(adList, oneday, channel, "头条");
        ...
        //头条
        channelVO.setToutiao(toutiao);
        Double toutiao_income = 0.0;
        for (int i = 0; i < toutiao.size(); i++) {
            AdDataVO adDataVO = toutiao.get(i);
            if (adDataVO != null) {
                adDataVO.setPerImpression(activity == 0 ? 0 : Judgement.formatDouble2((double) adDataVO.getImpressions() / activity));
                toutiao_income = toutiao_income + adDataVO.getIncome();
            }

        }
        Double toutiao_dauarpu = Judgement.formatDouble2(activity == 0 ? 0 : toutiao_income / activity);

        channelVO.setToutiao_income(toutiao_income);
        channelVO.setToutiao_dauarpu(Judgement.formatDouble2(toutiao_dauarpu));

        Double channel_income_all = guangdiantong_income + qudao_income + yixin_income + toutiao_income;
        channelVO.setChannel_income_all(channel_income_all);
        channelVO.setChhannel_dauarpu_all(Judgement.formatDouble2(activity == 0 ? 0 : channel_income_all / activity));
        return channelVO;
    }
     ...
     * @return ChannelVO
     */
    public ChannelVO ToChannelVO2(String name, List names, Long oneday, String channel, List<JsonObject> userList, List<JsonObject> adList, String plastform) {
        ChannelVO channelVO = new ChannelVO();
        String onedaysdf = Transform.transForDate(oneday);

        UserVO userVO = userJsonToUserVO1(userList, channel, oneday,names);
        List<AdDataVO> guang = adDataVOToAdDataVOlist1(names,adList, oneday, channel, "广点通");
        List<AdDataVO> qudao = adDataVOToAdDataVOlist1(names,adList, oneday, channel, plastform);
        List<AdDataVO> yixin = adDataVOToAdDataVOlist1(names,adList, oneday, channel, "移信");
        List<AdDataVO> toutiao = adDataVOToAdDataVOlist1(names,adList, oneday, channel, "头条");
     ...
     * @return
     */
    public List<List> listToTotalVOToList1(List<TotalVO> value) {
        List<List> aaaa = new ArrayList();
        for (int i = 0; i < value.size(); i++) {
            TotalVO totalVO = value.get(i);
            List list = new ArrayList<>();
            ...
            for (int j = 0; j < list1.size(); j++) {
                ...
                list.addAll(adDataListToLists(adDataVOS2));
                //头条
                list.add(Judgement.formatDouble2(channelVO.getToutiao_income()));
                list.add(userVO.getNewUser());
                list.add(userVO.getActivityUser());
                list.add(Judgement.toPercent(userVO.getRetention()));
                list.add(Judgement.NonScientificNotation(String.valueOf(Judgement.formatDouble4(channelVO.getToutiao_dauarpu()))));
                List<AdDataVO> adDataVOS3 = channelVO.getToutiao();
                list.addAll(adDataListToLists(adDataVOS3));
            }
            aaaa.add(list);
        }
        return aaaa;
