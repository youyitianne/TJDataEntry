...
//    }
     ...
     * @return jsonObjectList
     */
    public static List<JsonObject> countUserdata_dau(List<JsonObject> userlists, JsonObject nameJsonObject) {
        List<JsonObject> jsonObjects = new ArrayList<>();
        for (int i = 0; i < userlists.size(); i++) {
            String app_name = userlists.get(i).getString("app_name");
            Integer dau = userlists.get(i).getInteger("dau");
            ...
            if (jsonObjects.size() == 0) {
                JsonObject jsonObject = new JsonObject();
                if (nameJsonObject.getString(app_name)==null){
                    continue;
                }
                jsonObject.put("project", nameJsonObject.getString(app_name));
                jsonObject.put("date", date);
                ...
                for (int j = 0; j < jsonObjects.size(); j++) {
                    if (jsonObjects.get(j).getString("project").equals(nameJsonObject.getString(app_name)) &&
                            jsonObjects.get(j).getInteger("date").equals(date)) {
                        flag = false;
                        Integer original = jsonObjects.get(jsonObjects.size() - 1).getInteger("dau");
                        jsonObjects.get(jsonObjects.size() - 1).put("dau", original + dau);
                    }
     ...
     * @return jsonObjectList
     */
    public static List<JsonObject> countUserdata_dnu(List<JsonObject> userlists, JsonObject nameJsonObject) {
        List<JsonObject> jsonObjects = new ArrayList<>();
        for (int i = 0; i < userlists.size(); i++) {
            String app_name = userlists.get(i).getString("app_name");
            Integer dnu = userlists.get(i).getInteger("dnu");
            Integer date = userlists.get(i).getInteger("date");
            if (jsonObjects.size() == 0) {
                JsonObject jsonObject = new JsonObject();
                if (nameJsonObject.getString(app_name)==null){
                ...
                for (int j = 0; j < jsonObjects.size(); j++) {
                    if (jsonObjects.get(j).getString("project").equals(nameJsonObject.getString(app_name)) &&
                            jsonObjects.get(j).getInteger("date").equals(date)) {
                        flag = false;
                        Integer original = jsonObjects.get(jsonObjects.size() - 1).getInteger("dnu");
                        jsonObjects.get(jsonObjects.size() - 1).put("dnu", original + dnu);
                    }
                }
                if (flag) {
                    if (nameJsonObject.getString(app_name)==null){
    ...
    public static List<JsonObject> countAddata(List<JsonObject> adlists, JsonObject nameJsonObject) {
        ...
        for (int i = 0; i < adlists.size(); i++) {
            ...
            boolean flag = nameJsonObject.getString(app_name) == null || nameJsonObject.getString(app_name).length() <= 0;
            if (!flag) {
                if (jsonObjects.size() == 0) {
                    JsonObject jsonObject = new JsonObject();
                    jsonObject.put("project", nameJsonObject.getString(app_name));
